...
2. 새 보고서 폴더에 jsonl 저장 후 비교:
   ```bash
   python3 scripts/diff_surveys.py <직전_폴더> <새_폴더> --out new_items.jsonl
   ```
...
5. 보고서는 증분 구조로: **신규 공고 (A/B/C 분류) / 마감 변경 / 종료된 공고 중 직전 A그룹이던 것 (기회 소멸 알림) / 승계 요약 (직전 A그룹 현황 + 남은 마감)**. 직전 보고서 경로를 상단에 링크한다
...
python3 scripts/sources_crawl.py list bizinfo -o bizinfo.jsonl --max-pages 20
python3 scripts/sources_crawl.py list all -o sources_all.jsonl
```
...
**기업마당 주의**: 공고량이 많고(모집중 1,000건 이상) 마감 임박~지난 것이 섞여 나온다. `--max-pages`로 최근 등록분 위주로 끊고, 마감일 필터를 반드시 적용한다.
...
python3 scripts/sources_crawl.py detail <url> <url> ... -o details/
```
...
- **심사기준** — 단계별(서류·예선·본선) 배점표 원문 그대로, Pass/Fail 항목과 걸러내는 사유(예: "단순 제품·기업 제품 소개", "타 공모전 수상작", "진행 중인 타 공모전에서 수상하면 취소"). 공고 본문에 없으면 첨부 양식·FAQ·대회 홈페이지 '심사 기준' 메뉴까지 본다. 적합성 판정은 자격만이 아니라 **이 아이템이 배점표에서 어디서 점수를 받고 어디서 떨어지는지**로 한다 (2026-09-24 대표 "지원사업 볼 때 심사기준이 중요한 듯": 사회문제 해결 챌린지는 <브랜드> IR 을 그대로 내면 서류 적합성에서 탈락하는 구조였고, 부산 AI 경진은 1단계 시상이 현금이 아니었다)

신청서를 쓸 때(이 스킬 다음 단계)는 심사기준을 먼저 둔다: ①배점표를 원고 맨 위에 옮겨 적는다 ②칸 구성과 분량을 배점 비중에 맞추고, 칸마다 어느 항목 점수를 노리는지 정한다 ③쓰고 나면 심사위원 입장으로 항목별 자가 채점을 하고 약한 항목을 보강한다 ④외부 검수(코덱스) 프롬프트에도 배점표를 그대로 넣는다. 예시 = `<프로젝트>/01-<브랜드>\01-brand\02-tasks\13-지원사업\01-docs\코덱스_사회문제챌린지_신청서_검수_프롬프트.md`
...
- 저장 위치 (2026-08-17 3축 재편 반영): <브랜드> = `<프로젝트>/03-<계정>/02-tasks/06-지원사업/04-output/조사리포트/지원사업조사_<대상>_<YYYYMMDD>/`, <브랜드> = `<프로젝트>/01-<브랜드>/01-brand/02-tasks/13-지원사업/조사리포트/지원사업조사_<대상>_<YYYYMMDD>/` — 보고서 md + 원시 jsonl + 상세 원문. ~/Documents에 쓰지 말 것
